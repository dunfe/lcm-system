...
class SocketConnection {
  ...
  initializeSocketEvents = () => {
    ...
    this.socket.on('new-broadcast-messsage', (data: any) => {
      ...
      message.info(`${data.message.message} By ${data.userData.name}`);
    });
    ...
    return new Promise(resolve => {
      media.then((stream: MediaStream) => {
        ...
        if (type === 'displayMedia') {
          this.toggleVideoTrack({ audio, video } as MediaStatus);
          this.listenToEndStream(stream, { video, audio } as MediaStatus);
