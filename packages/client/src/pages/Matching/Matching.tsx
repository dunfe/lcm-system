...
const Matching = () => {
    ...
    const bind = useDrag(({args: [index, id], down, movement: [mx], direction: [xDir], velocity}) => {
        ...
        set((i) => {
            ...
            // TODO update api
            if (isGone && dir === 1) {
                instance.post(`/api/mentor/selectQuestion/${id}`, {
                    headers: {
    ...
    useEffect(() => {
        if (data) {
            const _cards = data.map((item) => {
                return (
                    <Descriptions title={item.title} key={item._id} className={'matching-description'} bordered>
                        <Descriptions.Item label="Người hỏi" span={3}>{item.menteeName}</Descriptions.Item>
                        <Descriptions.Item label="Point" span={3}>{item.point}</Descriptions.Item>
                        <Descriptions.Item label="Kỹ năng" span={3}>{item.skill.join(',')}</Descriptions.Item>
                        <Descriptions.Item label="Trạng thái">{item.status}</Descriptions.Item>
                        <Descriptions.Item label="Từ" span={2}>{dayjs(item.createAt).format('LLLL')}</Descriptions.Item>
