...
import { Table, Tag, Button, Space, message, Modal, Form, Input, InputNumber } from 'antd'
...
const Points = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render(text: string, record: any) {
                return (
                    <Space>
                        <Button type={'primary'} onClick={() => onAdd(record._id)}>
                            Add
                        </Button>
    ...
    const onUpdate = () => {
        form.validateFields().then((values) => {
            instance.post(`/api/staff/${mode === 'add' ? 'pointIn' : 'pointOut'}/${updateId}`, values).then((response) => {
    ...
    const expandedRowRender = () => {
        const expandColumns = [
            ...
            { title: 'Money', dataIndex: 'money', key: 'money', responsive: ['lg'] as Breakpoint[] },
            {
                title: 'Status',
                key: 'status',
                dataIndex: 'status',
            ...
            { title: 'Note', dataIndex: 'note', key: 'note', responsive: ['lg'] as Breakpoint[] },
        ]
    ...
    return (
        <>
            ...
            <Modal title={mode === 'add' ? 'Add Point' : 'Minus Point'} visible={isModalVisible}
                   footer={[
